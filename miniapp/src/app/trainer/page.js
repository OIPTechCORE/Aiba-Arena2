...
export default function TrainerPage() {
    ...
    useEffect(() => {
        if (tab === 'network') {
            const sort =
                networkSort === 'referred'
                    ? 'referred'
                    : networkSort === 'recruited'
                      ? 'recruited'
                      : networkSort === 'rewards'
                        ? 'rewards'
                        : 'impact';
    ...
    useEffect(() => {
        if (tab === 'leaderboard') {
            api.get('/api/trainers/leaderboard', {
                params: { by: leaderboardBy, period: leaderboardPeriod, limit: 50 },
                timeout: 8000,
            })
    ...
    async function claimRewards() {
        ...
        try {
            ...
            if (trainer)
                setTrainer({
                    ...trainer,
                    rewardsEarnedAiba: (trainer.rewardsEarnedAiba || 0) + (res.data?.claimedAiba || 0),
                });
        } catch (e) {
    ...
    return (
        <div style={{ padding: 24, maxWidth: 640, margin: '0 auto', minHeight: '100vh' }}>
            ...
            {loading && trainer === null ? (
                ...
                <div
                    style={{ padding: 16, border: '2px solid var(--accent-gold)', borderRadius: 12, marginBottom: 16 }}
                >
                    ...
                    <button
                        ...
                        onClick={() => applyTrainer(refCode || undefined)}
                        disabled={busy}
                    ...
                    {applyMsg ? (
                        <p style={{ marginTop: 8, color: 'var(--accent-green)', fontSize: 13 }}>{applyMsg}</p>
                    ) : null}
                </div>
                ...
                >
                    ...
                    <p style={{ color: 'var(--text-muted)', fontSize: 14 }}>
                        Your code: <strong>{trainer.code}</strong>. Browse the network while you wait.
                    </p>
                </div>
            ) : trainer?.status === 'suspended' ? (
            ...
            {trainer?.status !== 'suspended' && tab === 'network' ? (
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16, alignItems: 'center' }}>
                        ...
                        {['impact', 'referred', 'recruited', 'rewards'].map((b) => (
                            <button
                                key={b}
                                type="button"
                                onClick={() => setNetworkSort(b)}
                                style={{
                                    padding: '6px 12px',
                                    borderRadius: 6,
                                    border: `1px solid ${networkSort === b ? 'var(--accent-gold)' : 'var(--border)'}`,
                                    background: networkSort === b ? 'rgba(0,212,255,0.1)' : 'transparent',
                                    color: networkSort === b ? 'var(--accent-gold)' : 'var(--text)',
                                    textTransform: 'capitalize',
                                    fontSize: 13,
                                }}
                            >
                                    ...
                                    >
                                        <div>
                                            ...
                                            <div style={{ fontSize: 12, marginTop: 6 }}>
                                                Impact: {t.totalImpactScore ?? 0} · Referred: {t.referredUserCount ?? 0}{' '}
                                                · Recruited: {t.recruitedTrainerCount ?? 0} · Earned:{' '}
                                                {t.rewardsEarnedAiba ?? 0} AIBA
                                            </div>
                                        </div>
                                        <a
                                            href={`/trainer?ref=${t.code}`}
                                            style={{
                                                padding: '6px 12px',
                                                borderRadius: 6,
                        ...
                        {['alltime', 'monthly', 'weekly'].map((p) => (
                            <button
                                ...
                                style={{
                                    padding: '6px 12px',
                                    borderRadius: 6,
                                    border: `1px solid ${leaderboardPeriod === p ? 'var(--accent-gold)' : 'var(--border)'}`,
                                    background: leaderboardPeriod === p ? 'rgba(255,215,0,0.1)' : 'transparent',
                                    color: leaderboardPeriod === p ? 'var(--accent-gold)' : 'var(--text)',
                                    fontSize: 13,
                                }}
                            >
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16, alignItems: 'center' }}>
                        ...
                        {['impact', 'referred', 'recruited', 'rewards'].map((b) => (
                            <button
                                key={b}
                                type="button"
                                onClick={() => setLeaderboardBy(b)}
                                style={{
                                    padding: '6px 12px',
                                    borderRadius: 6,
                                    border: `1px solid ${leaderboardBy === b ? 'var(--accent-gold)' : 'var(--border)'}`,
                                    background: leaderboardBy === b ? 'rgba(255,215,0,0.1)' : 'transparent',
                                    color: leaderboardBy === b ? 'var(--accent-gold)' : 'var(--text)',
                                    textTransform: 'capitalize',
                                    fontSize: 13,
                                }}
                            >
                        ...
                        <div style={{ display: 'grid', gap: 8 }}>
                            {leaderboard.map((t, i) => {
                                ...
                                const periodVal = isPeriod
                                    ? leaderboardBy === 'referred'
                                        ? (t.periodReferred ?? 0)
                                        : leaderboardBy === 'recruited'
                                          ? (t.periodRecruited ?? 0)
                                          : (t.periodImpact ?? 0)
                                    : leaderboardBy === 'referred'
                                      ? (t.referredUserCount ?? 0)
                                      : leaderboardBy === 'recruited'
                                        ? (t.recruitedTrainerCount ?? 0)
                                        : leaderboardBy === 'rewards'
                                          ? (t.rewardsEarnedAiba ?? 0)
                                          : (t.totalImpactScore ?? 0);
                                const periodLabel =
                                    leaderboardBy === 'referred'
                                        ? `${periodVal} refs`
                                        : leaderboardBy === 'recruited'
                                          ? `${periodVal} recruited`
                                          : leaderboardBy === 'rewards' && !isPeriod
                                            ? `${periodVal} AIBA`
                                            : `Impact ${periodVal}`;
                                return (
                                    <div
                                        ...
                                        style={{
                                            ...
                                            borderRadius: 8,
                                            background:
                                                t.telegramId === trainer?.telegramId
                                                    ? 'rgba(0,212,255,0.08)'
                                                    : 'transparent',
                                        }}
                                    >
                                        ...
                                        <span>
                                            <strong>{t.displayName || t.username || t.code}</strong>
                                        </span>
                                        {t.specialty ? (
                ...
                <>
                    <div style={{ padding: 20, border: '1px solid var(--border)', borderRadius: 12, marginBottom: 20 }}>
                        ...
                        <p style={{ color: 'var(--text-muted)', fontSize: 14, marginBottom: 12 }}>
                            Other trainers see this in the Network. Optional.
                        </p>
                        <div style={{ display: 'grid', gap: 10 }}>
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    placeholder="Short intro for other trainers"
                                    rows={3}
                                    style={{
                                        width: '100%',
                                        padding: 10,
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="e.g. Global, EU, Asia"
                                    style={{
                                        width: '100%',
                                        padding: 10,
                                        marginTop: 4,
                            ...
                            <button type="button" className="btn btn--secondary" onClick={saveProfile} disabled={busy}>
                                Save profile
                            </button>
                            {profileMsg ? (
                                <p style={{ color: 'var(--accent-green)', fontSize: 13 }}>{profileMsg}</p>
                            ) : null}
                        </div>
                    </div>
                        ...
                        {trainer.tierMultiplierPercent != null && Number(trainer.tierMultiplierPercent) > 1 ? (
                            ...
                            >
                                <span style={{ fontWeight: 600, color: 'var(--accent-gold)' }}>
                                    Tier: {trainer.tierMultiplierPercent}x
                                </span>
                        ...
                        {trainer.seasonRank != null ? (
                            ...
                            >
                                <span style={{ fontWeight: 600, color: 'var(--text)' }}>
                                    Season rank: #{trainer.seasonRank}
                                </span>
                                <span style={{ fontSize: 12, color: 'var(--text-muted)', marginLeft: 6 }}>
                                    this month
                                </span>
                    ...
                    >
                        ...
                        <div style={{ display: 'grid', gap: 8, fontSize: 15 }}>
                            <div>
                                Referred users: <strong>{trainer.referredUserCount ?? 0}</strong>
                            </div>
                            <div>
                                Qualified (3+ battles): <strong>{trainer.referredUsersWithBattles ?? 0}</strong>
                            </div>
                            <div>
                                Trainers recruited: <strong>{trainer.recruitedTrainerCount ?? 0}</strong>
                            </div>
                            <div>
                                Rewards earned: <strong>{trainer.rewardsEarnedAiba ?? 0} AIBA</strong>
                            </div>
                        </div>
                        <button
                            type="button"
                    ...
                    <div style={{ padding: 20, border: '1px solid var(--border)', borderRadius: 12, marginBottom: 20 }}>
                        ...
                        <p style={{ color: 'var(--text-muted)', fontSize: 14 }}>
                            Share your link. When someone applies as trainer with your link, you earn 20 AIBA when they
                            get approved. Sharing unlocks milestones and boosts visibility.
                        </p>
                        {trainer.nextMilestone &&
                        (trainer.nextMilestone.referred != null || trainer.nextMilestone.recruited != null) ? (
                            <p style={{ fontSize: 13, color: 'var(--accent-gold)', marginBottom: 8 }}>
                                Next:{' '}
                                {[
                                    trainer.nextMilestone.referred != null
                                        ? `${trainer.nextMilestone.referred} referred`
                                        : null,
                                    trainer.nextMilestone.recruited != null
                                        ? `${trainer.nextMilestone.recruited} recruited`
                                        : null,
                                ]
                                    .filter(Boolean)
                                    .join(' or ')}
                    ...
                    {milestones.referred?.length > 0 || milestones.recruited?.length > 0 ? (
                        ...
                        >
                            ...
                            <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 10 }}>
                                Unlock badges and tier boosts by referring users and recruiting trainers.
                            </p>
                            <div style={{ display: 'grid', gap: 8 }}>
                                {(milestones.referred || []).map((m) => {
                                    ...
                                    const label =
                                        (milestones.labels?.referred || []).find((x) => x.value === val)?.label ||
                                        `${val} referred`;
                                    return (
                                        <div
                                            key={key}
                                        ...
                                        >
                                            <span
                                                style={{
                                                    color: unlocked ? 'var(--accent-green)' : 'var(--text-muted)',
                                                }}
                                            >
                                                {unlocked ? '✓' : '○'}
                                            </span>
                                            <span style={{ color: unlocked ? 'var(--text)' : 'var(--text-muted)' }}>
                                                {label}
                                            </span>
                                        </div>
                                    );
                                ...
                                {(milestones.recruited || []).map((m) => {
                                    ...
                                    const unlocked = (trainer.milestonesUnlocked || []).includes(key);
                                    const label =
                                        (milestones.labels?.recruited || []).find((x) => x.value === val)?.label ||
                                        `${val} recruited`;
                                    return (
                                        <div
                                            key={key}
