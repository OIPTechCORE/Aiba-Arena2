...
const BACKEND =
    ...
    (process.env.VERCEL ? DEFAULT_BACKEND : 'http://localhost:5000');
...
async function proxy(request, { params }, method) {
    ...
    const url =
        method === 'GET' && request.url?.includes('?') ? `${base}${request.url.slice(request.url.indexOf('?'))}` : base;
    try {
        ...
        return Response.json(
            { error: 'proxy_failed', message: err?.message || 'Proxy request failed' },
            { status: 502 },
        );
    }
}
