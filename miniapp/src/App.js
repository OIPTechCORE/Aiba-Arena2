...
function App() {
    ...
    useEffect(() => {
        // Auto-load brokers on page load (works in APP_ENV=dev via x-telegram-id fallback)
    ...
    useEffect(() => {
        ...
        // In production, backend should validate Telegram initData.
        // In dev mode, backend accepts telegramId from body.
    ...
    async function runBattle() {
        ...
        try {
            const res = await api.post('/api/battle/run', {
                ...
                arena: 'prediction',
                league: 'rookie',
                // telegramId only used in backend dev mode
    ...
    async function claimOnChain() {
        ...
        // Optional safety: if we know the vault inventory, block obviously impossible claims
        ...
        try {
            ...
            // Send a small TON amount to cover vault + jetton wallet gas.
            // Vault contract uses ~0.06 TON for the jetton transfer.
            await tonConnectUI.sendTransaction({
                validUntil: Math.floor(Date.now() / 1000) + 5 * 60,
                messages: [
                    {
                        address: claim.vaultAddress,
                        amount: '70000000', // 0.07 TON in nanotons
                        payload,
                    },
                ],
            });
            ...
            // Auto-check for confirmation (poll vault lastSeqno)
