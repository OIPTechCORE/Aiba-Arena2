...
async function verifyBottomNav(page) {
    const labels = await page.$$eval('.android-bottom-nav__label', (els) =>
    ...
    for (const r of required) {
        if (!labels.includes(r)) {
            inconsistencies.push({
                where: 'bottom-nav',
                expected: `Tab "${r}" present`,
                actual: `Not found. Labels: ${labels.join(', ')}`,
            });
        }
    }
...
async function verifyTabContent(page, tabId, tabLabel) {
    ...
    if (!panel)
        return { found: false, inconsistencies: [{ where: `tab-${tabId}`, expected: 'Active panel', actual: 'None' }] };
    ...
    try {
        ...
        for (const { id, label } of tabConfigs) {
            ...
            const card = await page
                .$('.tab-panel.is-active .card, .tab-panel.is-active .card--elevated')
                .catch(() => null);
        ...
        fs.writeFileSync(
            ...
            JSON.stringify({ tabs: report.tabs, measurements: report.measurements }, null, 2),
        );
