...
| 3 | **Redeploy** miniapp after changing env (NEXT_PUBLIC_* is baked at build time). |
...
| **Backend won’t start** | Missing or invalid env; prod checks fail. | Check Vercel build/logs for PROD_READINESS_FAILED. Set all required backend env (MONGO_URI, CORS_ORIGIN, TELEGRAM_BOT_TOKEN, ADMIN_*, BATTLE_SEED_SECRET, PUBLIC_BASE_URL). |
