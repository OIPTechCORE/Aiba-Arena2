...
- **TonConnect (Connect Wallet):** Clicking **Connect Wallet** in the miniapp header opens a modal with the list of TON-supported wallets (Tonkeeper, TonHub, etc.); the modal is responsive (safe-area, full viewport on mobile). The app manifest is served at `http://localhost:3000/api/tonconnect-manifest`. Many wallets (especially mobile) cannot load it from localhost. For local testing: use a browser wallet extension, or set `NEXT_PUBLIC_TONCONNECT_MANIFEST_URL=https://aiba-arena2.vercel.app/api/tonconnect-manifest` in `.env.local`, or use an HTTPS tunnel (e.g. ngrok).
