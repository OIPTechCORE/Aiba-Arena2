...
18. **Rewards** — proposedAiba = score * baseRewardAibaPerScore * mode.rewardMultiplierAiba (then innovations/boost). proposedNeur similarly. `tryEmitNeur` / `tryEmitAiba` apply daily caps and emission windows. Actual credited amounts = rewardNeur, rewardAiba.
...
- **Variance:** varianceBase (default 30, or rules.varianceBase) * leagueMul * (0.2 + risk).  
