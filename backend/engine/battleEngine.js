...
function simulateBattle({ broker, seed, arena }) {
    ...
    const arenaWeights =
        arena === 'prediction'
            ...
            : arena === 'arbitrage'
              ? { intelligence: 0.4, speed: 0.4, risk: 0.2 }
              : { intelligence: 0.5, speed: 0.3, risk: 0.2 };

    const base = 100 * (arenaWeights.intelligence * intelligence + arenaWeights.speed * speed + arenaWeights.risk * risk);

    // risk adds variance
    const variance = 30 * (0.2 + risk);
