 ...
 */
router.get('/', validateQuery({ limit: { type: 'integer', min: 1, max: 100 } }), async (req, res) => {
    try {
        const limit = getLimit({ query: { limit: req.validatedQuery?.limit } }, { defaultLimit: 50, maxLimit: 100 });
        const items = await Announcement.find().sort({ createdAt: -1 }).limit(limit).lean();
        res.json(items);
    } catch (err) {
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            const type = ['announcement', 'maintenance', 'status'].includes(
...
router.patch(
    ...
    async (req, res) => {
        try {
            ...
            if (
                req.validatedBody?.type !== undefined &&
                ['announcement', 'maintenance', 'status'].includes(String(req.validatedBody.type).trim())
            ) {
 ...
 */
router.post('/:id/broadcast', validateParams({ id: { type: 'objectId', required: true } }), async (req, res) => {
    try {
