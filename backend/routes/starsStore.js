...
router.post(
    ...
    try {
        ...
        const wallet = (process.env.STARS_STORE_WALLET || '').trim();
        if (!wallet) return res.status(503).json({ error: 'Stars store TON wallet not configured (STARS_STORE_WALLET)' });
        ...
        if (existing) return res.status(409).json({ error: 'this transaction was already used for a Stars purchase' });
