...
router.patch(
    ...
    async (req, res) => {
        try {
            const aibaPerTon =
                req.validatedBody?.oracleAibaPerTon !== undefined
                    ? Number(req.validatedBody.oracleAibaPerTon)
                    : undefined;
            const neurPerAiba =
                req.validatedBody?.oracleNeurPerAiba !== undefined
                    ? Number(req.validatedBody.oracleNeurPerAiba)
                    : undefined;
            const update = {};
            ...
            if (Object.keys(update).length === 0)
                return res.status(400).json({ error: 'oracleAibaPerTon or oracleNeurPerAiba required' });
