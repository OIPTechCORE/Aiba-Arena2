...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            if (!['intelligence', 'speed', 'risk'].includes(stat))
    ...
    async (req, res) => {
        try {
            ...
            if (!baseId || !sacrificeId)
                return res.status(400).json({ error: 'baseBrokerId and sacrificeBrokerId required' });
            if (baseId === sacrificeId)
                return res.status(400).json({ error: 'base and sacrifice must be different brokers' });

            const [base, sacrifice] = await Promise.all([Broker.findById(baseId), Broker.findById(sacrificeId)]);
            ...
            if (String(sacrifice.ownerTelegramId) !== telegramId)
                return res.status(403).json({ error: 'not your broker' });
