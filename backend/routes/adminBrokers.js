...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            const nftItemIndex =
                nftItemIndexRaw === undefined || nftItemIndexRaw === null ? null : Number(nftItemIndexRaw);
            const metadataUri =
                req.validatedBody?.metadataUri !== undefined
                    ? String(req.validatedBody.metadataUri || '').trim()
                    : undefined;
...
router.get('/mint-jobs', validateQuery({ status: { type: 'string', trim: true, maxLength: 20 } }), async (req, res) => {
    try {
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            const nftItemAddress = String(req.validatedBody?.nftItemAddress || '').trim();
            const nftItemIndex =
                req.validatedBody?.nftItemIndex != null ? Number(req.validatedBody.nftItemIndex) : null;
            if (!nftCollectionAddress || !nftItemAddress)
                return res.status(400).json({ error: 'nftCollectionAddress and nftItemAddress required' });
