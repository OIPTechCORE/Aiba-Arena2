    ...
    try {
        ...
        if (!canCreateFree) {
            if (costNano <= 0 || !leaderBoardWallet)
                return res.status(400).json({ error: 'Creating a group requires being in top ' + topFree + ' or paying TON (not configured)' });
            if (!txHash) return res.status(400).json({ error: 'txHash required — pay ' + (costNano / 1e9) + ' TON to create a group' });
...
router.post('/:guildId/boost', async (req, res) => {
    ...
    try {
        ...
        if (boostTxHashes.includes(txHash)) return res.status(409).json({ error: 'txHash already used for this group' });
