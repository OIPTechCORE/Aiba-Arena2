...
async function runCarRaceIfFull(raceId) {
    ...
    const sumBonuses = results.length > 0 ? results.reduce((s, _, i) => s + (positionBonus[i] ?? 0.5), 0) : 0;
    race.status = 'running';
...
router.get(
    ...
    async (req, res) => {
        try {
            const limit = getLimit({ query: { limit: req.validatedQuery?.limit } }, { defaultLimit: 20, maxLimit: 50 });
            const list = await CarListing.find({ status: 'active' })
                .sort({ createdAt: -1 })
                .limit(limit)
                .populate('carId')
                .lean();
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            if (!carId || !Number.isFinite(priceAIBA) || priceAIBA < 0)
                return res.status(400).json({ error: 'carId and priceAIBA required' });
            const car = await RacingCar.findById(carId);
            if (!car) return res.status(404).json({ error: 'car not found' });
            if (String(car.ownerTelegramId) !== telegramId) return res.status(403).json({ error: 'not your car' });
