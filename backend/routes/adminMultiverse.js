...
router.get(
    ...
    async (req, res) => {
        try {
            ...
            const universeSlug = req.validatedQuery?.universeSlug
                ? String(req.validatedQuery.universeSlug).trim()
                : null;
            ...
            const list = await NftStake.find(q)
                .populate('brokerId', 'ownerTelegramId level nftItemAddress')
                .sort({ stakedAt: -1 })
                .limit(limit)
                .lean();
