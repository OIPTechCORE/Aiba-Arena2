...
const COURSES = [
    {
        ...
        modules: [
            {
                id: 'what-is',
                title: 'What is AIBA Arena?',
                body: 'AIBA Arena is a Telegram Mini App where you own AI brokers (trading agents with stats), enter arenas and leagues, run battles, and earn NEUR and AIBA. Play with brokers in arenas → get score → earn rewards → optionally withdraw AIBA to your TON wallet.',
            },
            {
                id: 'brokers',
                title: 'Brokers',
                body: 'A broker is your in-game agent. Each has Intelligence, Speed, and Risk (0–100), plus level, energy, and cooldowns. You can have multiple brokers. Create a starter broker from the Brokers tab, then pick one for each battle.',
            },
            {
                id: 'first-battle',
                title: 'Your first battle',
                body: 'On Home: select a broker and an arena (e.g. prediction). Hit Run battle. The server runs a deterministic simulation and returns a score. You earn NEUR and AIBA credits based on score (within daily caps and emission windows).',
            },
            {
                id: 'wallet-claim',
                title: 'Connect wallet & claim',
                body: 'Connect your TON wallet (TonConnect). AIBA credits can be withdrawn on-chain: after a battle, use Create claim or enable Auto-claim, then send the signed transaction to the ArenaRewardVault to receive AIBA jettons.',
            },
        ],
    },
    {
        ...
        modules: [
            {
                id: 'arenas',
                title: 'Arenas',
                body: 'Prediction (intelligence-weighted), Simulation (balanced), Strategy Wars (risk matters more), Guild Wars (requires a guild; rewards split with guild treasury). Each arena has different stat weights and meta.',
            },
            {
                id: 'leagues',
                title: 'Leagues',
                body: 'Rookie, Pro, Elite. Higher league usually means higher score multiplier and may require a minimum broker level. Energy and cooldown costs can vary by mode.',
            },
            {
                id: 'guild-wars',
                title: 'Guild Wars',
                body: 'Requires guild membership. You can deposit a broker into the guild pool. Rewards are split (e.g. 80% to you, 20% to guild treasury). Join or create a group from the Guilds tab.',
            },
        ],
    },
    {
        ...
        modules: [
            {
                id: 'neur',
                title: 'NEUR',
                body: 'Off-chain currency. Earned from battles and referrals. Spent on entry fees, upgrades, training, repairs. Stored in the backend; no on-chain withdrawal.',
            },
            {
                id: 'aiba',
                title: 'AIBA credits & on-chain claim',
                body: 'AIBA balance = off-chain credits. Earned from battles. Withdraw: create a signed claim (after battle or from Wallet), then send one TonConnect tx to the ArenaRewardVault to receive AIBA jettons on TON.',
            },
            {
                id: 'stars-diamonds',
                title: 'Stars & Diamonds',
                body: 'Stars: Telegram Stars–style in-app currency. Earn from every battle win. Diamonds: rare TON ecosystem asset; earned on your first battle win. Both shown in Wallet and balance strip.',
            },
            {
                id: 'staking',
                title: 'Staking',
                body: 'Lock AIBA in the Yield Vault, earn APY. Unstake or claim rewards anytime from the Wallet tab. Configurable APY in economy config.',
            },
        ],
    },
    {
        ...
        modules: [
            {
                ...
                body: 'Guilds are groups. Create one (name, bio) or join by Guild ID. Top leaders by score can create a group for free; others may pay TON. Deposit brokers into the guild pool for guild wars.',
            },
            {
                id: 'guild-rewards',
                title: 'Guild Wars rewards',
                body: 'When you run a battle in Guild Wars, a share of NEUR goes to the guild treasury. The guild vault grows from member activity.',
            },
            {
                id: 'groups-leaderboard',
                title: 'Groups leaderboard',
                body: 'All groups are visible. Your leaderboard rank (by total score) determines whether you can create a group for free. Check the Guilds tab for your rank and discover groups.',
            },
        ],
    },
    {
        ...
        modules: [
            {
                ...
                body: 'Keep brokers energized; pick the right arena for your broker stats (e.g. high INT for Prediction). Use referrals to earn bonus NEUR. Connect wallet early so you can claim AIBA on-chain when ready.',
            },
            {
                id: 'energy-cooldowns',
                title: 'Energy & cooldowns',
                body: 'Each battle consumes energy; it regenerates over time (e.g. 1 per minute, cap 100). Cooldowns prevent spamming the same arena. Plan battles around energy and cooldown.',
            },
            {
                id: 'stake-mint',
                title: 'When to stake, mint NFT',
                body: 'Stake AIBA to earn APY with no lock period. Mint a broker as NFT (costs AIBA) to get an on-chain collectible; useful for ownership proof or future marketplace.',
            },
            {
                id: 'wall-of-fame',
                title: 'Wall of Fame',
                body: 'Leaderboards rank players by score, AIBA, NEUR, or battles. Badges (e.g. verified, top leader) appear on profiles and leaderboards. Climb the ranks and earn recognition.',
            },
        ],
    },
    {
        ...
        modules: [
            {
                id: 'referrals',
                title: 'Referrals',
                body: 'Go to the Referrals tab: create your code (My code), share it. When someone applies your code at signup or in-app, you both get bonuses (NEUR or AIBA if configured). Top referrers appear on the leaderboard.',
            },
            {
                id: 'car-bike-racing',
                title: 'Car & Bike Racing',
                body: 'Create or buy a car or bike with AIBA or TON. Enter open races from the Car Racing or Bike Racing tabs. Earn AIBA by finish position. The system shop sells cars/bikes for AIBA.',
            },
            {
                id: 'market',
                title: 'Market',
                body: 'Sell a broker for AIBA or buy one from players or from the system. Withdraw your broker from the guild pool first if it is deposited there—only brokers not in a guild pool can be listed.',
            },
            {
                id: 'guild-withdrawal',
                title: 'Guild deposit & withdrawal',
                body: 'Deposit brokers into the guild pool for Guild Wars. To sell a deposited broker on the Market, withdraw it from the guild first from the Guilds tab.',
            },
        ],
    },
];
        ...
        try {
            ...
            if (completedKeys.includes(key)) {
                const summary = {
                    ...
                    totalModules,
                    graduate: completedKeys.length >= totalModules,
                };
            ...
            if (nowComplete) {
                await User.updateOne({ telegramId }, { $addToSet: { badges: 'university_graduate' } }).catch(() => {});
            }
    ...
    async (req, res) => {
        try {
            ...
            if (!wallet)
                return res
                    .status(503)
                    .json({ error: 'Course badge mint wallet not configured (UNIVERSITY_BADGE_TON_WALLET)' });
            ...
            if (completedCourseIds.length < 1)
                return res
                    .status(403)
                    .json({ error: 'Complete at least one course to mint the Course Completion badge' });
            ...
            await User.updateOne({ telegramId }, { $addToSet: { badges: 'course_completion' } });
    ...
    async (req, res) => {
        try {
            ...
            if (!wallet)
                return res
                    .status(503)
                    .json({ error: 'University certificate mint wallet not configured (UNIVERSITY_BADGE_TON_WALLET)' });
            ...
            if (!graduate)
                return res
                    .status(403)
                    .json({ error: 'Complete all courses to mint the Full Course Completion Certificate' });
            ...
            await User.updateOne({ telegramId }, { $addToSet: { badges: 'full_course_completion_certificate' } });
