...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            if (costNano <= 0 || !giftsWallet) return res.status(503).json({ error: 'Gifts not configured' });
...
router.get('/received', validateQuery({ limit: { type: 'integer', min: 1, max: 50 } }), async (req, res) => {
    try {
...
router.get('/sent', validateQuery({ limit: { type: 'integer', min: 1, max: 50 } }), async (req, res) => {
    try {
        const telegramId = String(req.telegramId || '');
        const limit = getLimit({ query: { limit: req.validatedQuery?.limit } }, { defaultLimit: 20, maxLimit: 50 });
