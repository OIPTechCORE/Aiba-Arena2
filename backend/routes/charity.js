    ...
    async (req, res) => {
        try {
            ...
            const list = await CharityCampaign.find(query)
                ...
                .lean()
                .select(
                    'name description cause goalNeur goalAiba goalTonNano raisedNeur raisedAiba raisedTonNano donorCount status endAt featured',
                );
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            if (!result.ok) {
                const status = result.error === 'insufficient_neur' || result.error === 'insufficient_aiba' ? 403 : 400;
