    ...
    async (req, res) => {
        try {
            ...
            if (!debit.ok)
                return res
                    .status(403)
                    .json({ error: debit.reason === 'insufficient' ? 'insufficient AIBA' : 'debit failed' });
                    ...
                    getCreatorReferrerAndBps(entry.telegramId, cfg).then(async (creator) => {
                        ...
                        if (creatorAiba > 0) {
                            await creditAibaNoCap(creatorAiba, {
                                ...
                                sourceId: String(race._id),
                                meta: {
                                    refereeTelegramId: entry.telegramId,
                                    bps: creator.bps,
                                    amountAiba: creatorAiba,
                                },
                            });
                        }
                    }),
...
router.get('/race/:id', validateParams({ id: { type: 'objectId', required: true } }), async (req, res) => {
    try {
...
router.get('/leaderboard', validateQuery({ limit: { type: 'integer', min: 1, max: 100 } }), async (req, res) => {
    try {
        const limit = getLimit({ query: { limit: req.validatedQuery?.limit } }, { defaultLimit: 20, maxLimit: 100 });
...
router.get('/system-bikes', (_req, res) => {
    try {
        const withLabel = SYSTEM_BIKES.map((b) => ({
            ...b,
            classLabel: BIKE_CLASS_LABELS[b.bikeClass] || b.bikeClass,
        }));
        res.json(withLabel);
    } catch (err) {
...
router.get(
    ...
    async (req, res) => {
        try {
            const limit = getLimit({ query: { limit: req.validatedQuery?.limit } }, { defaultLimit: 20, maxLimit: 50 });
            const list = await BikeListing.find({ status: 'active' })
                .sort({ createdAt: -1 })
                .limit(limit)
                .populate('bikeId')
                .lean();
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            if (!bikeId || !Number.isFinite(priceAIBA) || priceAIBA < 0)
                return res.status(400).json({ error: 'bikeId and priceAIBA required' });
            const bike = await RacingMotorcycle.findById(bikeId);
            if (!bike) return res.status(404).json({ error: 'bike not found' });
            if (String(bike.ownerTelegramId) !== telegramId) return res.status(403).json({ error: 'not your bike' });
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            if (String(listing.sellerTelegramId) === telegramId)
